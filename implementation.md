...
Aria is a **WhatsApp Commerce Platform for Indonesian SMB merchants**. Merchants connect their WhatsApp Business API (Meta WABA) account and get:
...
```
.
├── Aria_Flow_Engine_PRD_v2.1.md   # North star — read this first
├── apps/
...
│   │       │   └── webhooks/         # meta.ts, midtrans.ts, xendit.ts
...
├── packages/
│   ├── ai/                           # @aria/ai — LLM clients (Grok/xAI), RAG pipeline
│   ├── db/                           # @aria/db — Drizzle ORM, schema, migrations
│   ├── flow-engine/                  # @aria/flow-engine — core runtime (see §8-9)
│   ├── meta/                         # @aria/meta — MetaClient (WhatsApp Graph API)
│   ├── pantheon/                     # @aria/pantheon — buyer genome / analytics
│   ├── payments/                     # @aria/payments — Midtrans + Xendit
│   ├── shared/                       # @aria/shared — constants (QUEUES, etc.)
...
```bash
pnpm -F @aria/shared build
pnpm -F @aria/db build
pnpm -F @aria/meta build
pnpm -F @aria/flow-engine build
pnpm -F @aria/ai build || true        # optional; skip if not needed
pnpm -F @aria/pantheon build || true  # optional
pnpm -F @aria/payments build || true  # optional
pnpm -F  build || true      # optional
pnpm -F api build
pnpm -F worker build
...
**Migrations:** `packages/db/src/migrations/` — run via `pnpm -F @aria/db migrate`. Runs automatically on API startup (`apps/api/src/migrate.ts`).

### Pre-existing Tables (before Flow Engine)

| Table | Schema File | Key Columns |
|-------|-------------|-------------|
| `tenants` | `tenants.ts` | `id, lynkUserId, storeName, wabaId, metaAccessTokenEnc, subscriptionTier (trial\|growth\|pro\|scale), metaAccessToken (encrypted), messagingTier (int), wabaQualityRating, lastRiskScoreAt` |
...
|-----------------|------------|----------------|------------|--------------|
| `QUEUES.INGEST` | `aria-ingest` | `ingest.processor.ts` | 2 (lockDuration=5m) | PDF ingestion + LLM chunking |
| `QUEUES.TRACKING` | `aria-tracking` | `tracking.processor.ts` | 10 | Shipment tracking updates |
| `QUEUES.PAYMENT_EXPIRY` | `aria-payment-expiry` | `paymentExpiry.processor.ts` | 5 | Order expiry |
| `QUEUES.STOCK_RELEASE` | `aria-stock-release` | `stockRelease.processor.ts` | 5 | Return stock to inventory |
| `QUEUES.RESTOCK_NOTIFY` | `aria-restock-notify` | `restock.processor.ts` | 5 | Restock alerts |
| `QUEUES.FLOW_EXECUTION` | `aria-flow-execution` | `flowExecution.processor.ts` | 20 (lockDuration=60s) | Flow node execution + delays |
| `QUEUES.TEMPLATE_SYNC` | `aria-template-sync` | `templateSync.processor.ts` | 5 | Template status polling + quality sync |
| `QUEUES.RISK_SCORE` | `aria-risk-score` | `riskScore.processor.ts` | 3 | Tenant risk score computation |
...
```typescript
import { db, flowDefinitions, eq, and } from '@aria/db';
...
vi.mock('@aria/db', () => ({ db: mockDb, eq: vi.fn(), and: vi.fn(), ... }));
vi.mock('@aria/ai', () => ({ getLLMClient: () => mockLLM }));
```
...
```bash
docker build -f Dockerfile.test -t aria-test .
docker run aria-test sh -c "cd apps/api && npx vitest run"
```
...
```bash
pnpm -F @aria/flow-engine typecheck   # Zero errors required
...
These files have pre-existing type errors from unbuilt optional packages (`@aria/ai`, `@aria/pantheon`, `@aria/payments`). These are expected and not new:
...
|----------------|--------|---------------|---------------|
| `aria-api` | monorepo root | `pnpm install && pnpm -F @aria/shared build && ... && pnpm -F api build` | `pnpm -F api start` |
| `aria-worker` | monorepo root | same build order → `pnpm -F worker build` | `pnpm -F worker start` |
| `aria-dashboard` | monorepo root | `pnpm install && pnpm -F dashboard build` | `pnpm -F dashboard preview` |

### Migration Strategy

Set **Release Command** on `aria-api`:
```
pnpm -F @aria/db migrate
```
...
These are intentional divergences from `Aria_Flow_Engine_PRD_v2.1.md`. Do not "fix" these — they are decisions made during implementation.
...
#### Stage 6 — Is `wabaStatus` = `'active'`?

`GET /api/v1/onboarding/status` returns `onboarded = (wabaStatus === 'active' && !!metaPhoneNumberId)`. Completing onboarding via Settings sets `wabaStatus = 'active'`. If it's still `null` or `'pending'`, the dashboard shows "Not connected" and the AI state machine won't respond (though the webhook will still save messages).
...
4. Run `pnpm -F @aria/db migrate` or use Railway Release Command
