...
|----------------|--------|---------------|
| `aria-api` | `apps/api` | `pnpm -F api start` |
| `aria-worker` | `apps/worker` | `pnpm -F worker start` |
| `aria-dashboard` | `apps/dashboard` | `pnpm -F dashboard build && pnpm -F dashboard preview` |
...
META_WEBHOOK_VERIFY_TOKEN=aria_webhook_verify   # or choose your own
...
1. **PostgreSQL plugin** → connect to both `aria-api` and `aria-worker`  
   Railway injects `DATABASE_URL` automatically.

2. **Redis plugin** → connect to both `aria-api` and `aria-worker`  
...
|---------|---------------|---------------|---------------|
| `aria-api` | `/` (monorepo root) | `pnpm install && pnpm -F api build` | `pnpm -F api start` |
| `aria-worker` | `/` | `pnpm install && pnpm -F worker build` | `pnpm -F worker start` |
| `aria-dashboard` | `/` | `pnpm install && pnpm -F dashboard build` | `pnpm -F dashboard preview` |

> **Note:** The build command for `api` and `worker` must also build workspace dependencies in order:
> `pnpm -F @aria/shared build && pnpm -F @aria/db build && pnpm -F @aria/meta build && pnpm -F @aria/flow-engine build && pnpm -F @aria/ai build && pnpm -F @aria/pantheon build && pnpm -F @aria/payments build && pnpm -F api build`
...
railway run --service aria-api pnpm -F @aria/db migrate
...
#    Check Railway logs for "aria-flow-execution" queue activity
```
...
| Queue backlogs | BullMQ Bull-Board (optional), or query Redis `LLEN aria-*` |
