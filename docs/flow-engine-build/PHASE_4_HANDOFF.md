...
4. **`computeRiskScore` import in AI route**: Import from `@aria/flow-engine`, same as in `flows.ts`.

5. **Pre-existing typecheck errors**: `@aria/ai`, `@aria/pantheon`, `@aria/payments` packages are not built in Docker. Errors in `ai.ts`, `intelligence.ts`, `conversation.service.ts`, `payment.service.ts` are pre-existing — do NOT fix them in Phase 5.
...
All must pass in Docker (`aria-test:phase5`):

```bash
pnpm -F @aria/flow-engine build  # should already pass; rebuild after prompts file added
...
`pnpm -F api exec vitest run` (flowTemplates + crypto): **28/28 passed** in Docker (aria-test:phase4)

### Typecheck Status

- `pnpm -F api typecheck`: zero errors in Phase 4 files
- `pnpm -F worker typecheck`: zero errors in Phase 4 files
- `pnpm -F @aria/flow-engine build`: clean (image builds without error)
