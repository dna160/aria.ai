...
- **Deleted**: `DEPLOYMENT.md`, `wabaStatus.processor.ts`
...
All tests use `vitest`. Mock `@aria/db`, `@aria/meta`, `bullmq`.
...
pnpm -F @aria/flow-engine typecheck
pnpm -F @aria/flow-engine test    # all unit tests pass
pnpm -F api typecheck                # zero errors
pnpm -F worker typecheck             # zero errors
pnpm -F @aria/flow-engine lint
```
...
- `apps/api/package.json` — added `@aria/flow-engine: workspace:*`
...
- `apps/worker/package.json` — added `@aria/flow-engine: workspace:*`
...
- `pnpm -F @aria/flow-engine build` — PASS
- `pnpm -F @aria/flow-engine typecheck` — PASS
- `pnpm -F @aria/flow-engine test` — PASS (77/77)
...
4. **`computeRiskScore`** is exported from `@aria/flow-engine` — Phase 4 should import it from there, not re-implement.
