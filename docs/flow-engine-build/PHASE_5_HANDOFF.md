...
- Fix `conversation.service.test.ts` (incomplete `@aria/db` mock — missing `eq` and `and`)
...
The mock at the top of the file does not export `eq` and `and` from `@aria/db`.
The service calls `eq(...)` and `and(...)` as WHERE helpers; they need to be mocked.

**Fix** — add to the `vi.mock('@aria/db', () => ({...}))` factory:
```typescript
...
`pnpm -F api exec vitest run` (flowTemplates + crypto): **28/28 passed** in Docker (aria-test:phase5)
...
Applied after audit against `Aria_Flow_Engine_PRD_v2.1.md`:
