...
- Delete dead code: `infra/DEPLOYMENT.md` AND root `DEPLOYMENT.md` (whichever exist), `apps/worker/src/processors/wabaStatus.processor.ts`
...
- `packages/db/src/schema/tenants.ts` already has: `id, lynkUserId, storeName, wabaId, metaAccessTokenEnc, wabaStatus, metaPhoneNumberId, displayPhoneNumber, subscriptionTier (trial|growth|pro|scale enum), subscriptionExpiresAt, createdAt, updatedAt`
...
- `apps/worker/src/processors/wabaStatus.processor.ts` — confirmed no-op stub
...
4. Update `tenants` row → set `metaPhoneNumberId`, `wabaId`, `metaAccessToken` (encrypted), `wabaStatus = 'active'`
...
- Manual path: validate the three creds are non-empty; encrypt `metaAccessToken` with `WABA_POOL_ENCRYPTION_KEY`; write to tenant row; set `wabaStatus = 'active'`.
...
GET  /v1/onboarding/status     { onboarded: boolean, mode: 'pool'|'manual'|null, displayPhone?: string, wabaStatus: string }
```
...
- REMOVE `WATI_STATUS`
- ADD: `FLOW_EXECUTION: 'aria-flow-execution'`, `TEMPLATE_SYNC: 'aria-template-sync'`, `RISK_SCORE: 'aria-risk-score'`

Modify `apps/worker/src/index.ts`:
- REMOVE the `wabaStatus` processor registration
...
- DELETE `apps/worker/src/processors/wabaStatus.processor.ts`
...
- `pnpm install`
- `pnpm -w build` (or `pnpm -F @aria/db build && pnpm -F @aria/meta build && pnpm -F api build && pnpm -F dashboard build && pnpm -F worker build`)
- `pnpm -w typecheck`
- `pnpm -w lint`
...
apps/worker/src/index.ts                       — remove wabaStatus
...
apps/worker/src/processors/wabaStatus.processor.ts
```
