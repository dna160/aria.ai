...
Added `pnpm -F @aria/payments build || true` and `pnpm -F  build || true` so the full workspace is resolvable when running the test container in CI.
...
- `Dockerfile.test` — added `@aria/payments` + `` build steps
