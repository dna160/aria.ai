...
#
# Railway setup:
#   aria-api    → no config needed (SERVICE=api is the default)
#   aria-worker → set Build Variable: SERVICE=worker
...
# ─── Stage 1: Pruner ─────────────────────────────────────────────────────────
# turbo prune extracts only the packages @aria/$SERVICE needs, with a
# trimmed pnpm-lock.yaml — this is what makes the build fast and reliable.
FROM alpine AS pruner
ARG SERVICE
WORKDIR /app
COPY . .
RUN turbo prune @aria/${SERVICE} --docker
...
RUN turbo run build --filter=@aria/${SERVICE}...
