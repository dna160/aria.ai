 ...
 * Role    : Base system prompt builder for Aria AI persona.
 ...
 * DO NOT  : Import from apps/*, payments
 */
...
export function buildSystemPrompt(ctx: SystemPromptContext): string {
  const baseID = `Kamu adalah Aria, asisten penjualan WhatsApp cerdas untuk toko "${ctx.storeName}".
...
- Hormati privasi pembeli`;
  ...
  const baseEN = `You are Aria, an intelligent WhatsApp sales assistant for "${ctx.storeName}".
