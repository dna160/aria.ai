...
class ErrorBoundary extends Component<{ children: ReactNode }, EBState> {
  ...
  componentDidCatch(error: Error, info: { componentStack: string }) {
    console.error('[Aria ErrorBoundary]', error, info);
  }
      ...
      <div className="min-h-screen bg-[#0F172A] flex items-center justify-center p-6 text-white">
        <div className="w-full max-w-2xl space-y-5">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => { localStorage.removeItem('aria_token'); window.location.href = '/login'; }}
          ...
          <p className="text-slate-500 text-xs">
            ...
            <code className="bg-white/10 px-1 rounded">cat /tmp/aria-api.log | tail -50</code>{' '}
            for API errors.
          </p>
        </div>
      </div>
