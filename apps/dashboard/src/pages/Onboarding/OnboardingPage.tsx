    ...
    <div className="min-h-screen bg-[#0F172A] flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-2xl">
        <div className="text-center mb-8">
          <span className="text-3xl font-bold text-indigo-400">Aria</span>
          <p className="text-slate-400 mt-1 text-sm">Set up your WhatsApp AI store</p>
        </div>
        <div className="mb-8">
          <div className="flex justify-between mb-2">
            {STEPS.map((s, i) => <span key={i} className={`text-xs ${i + 1 <= step ? 'text-indigo-400' : 'text-slate-500'}`}>{s.label}</span>)}
          </div>
