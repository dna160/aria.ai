...
vi.mock('@aria/db', () => ({
  ...
  messages: { id: 'id', metaMessageId: 'metaMessageId', conversationId: 'conversationId' },
...
import { MetaClient, extractText, isLocationMessage } from '@aria/meta';
...
describe('ConversationService', () => {
  ...
  beforeEach(() => {
    ...
    (db.query.tenants.findFirst as any).mockResolvedValue({ id: 'tenant-1', storeName: 'Test Store', metaAccessTokenEnc: 'plainkey' });
  ...
  describe('idempotency', () => {
    it('processes the same messageId only once', async () => {
      ...
      (db.query.messages.findFirst as any).mockResolvedValueOnce({ id: 'existing', metaMessageId: messageId });
      await service.handleInbound('tenant-1', payload);
  ...
  describe('isDuplicate', () => {
    ...
    it('returns true when messageId already exists', async () => {
      (db.query.messages.findFirst as any).mockResolvedValue({ id: 'x', metaMessageId: 'dup-id' });
