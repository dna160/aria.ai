 ...
 * Imports : @aria/shared, @aria/db, @aria/ai, @aria/meta
 ...
 */
import { eq, and, gt } from '@aria/db';
import { db, conversations, messages, buyers, tenants, products, waitlist, buyerGenomes } from '@aria/db';
import {
  BUY_INTENT_KEYWORDS,
  OBJECTION_KEYWORDS,
...
import type { ConversationStateValue } from '@aria/shared';
...
export class ConversationService {
  ...
  async sendAiResponse(
    ...
    const systemPrompt = buildSystemPrompt({
      storeName: tenant?.storeName ?? 'Aria Store',
  ...
  async isDuplicate(messageId: string): Promise<boolean> {
    ...
    const existing = await db.query.messages.findFirst({
      where: eq(messages.metaMessageId, messageId),
    });
    return !!existing;
  }
